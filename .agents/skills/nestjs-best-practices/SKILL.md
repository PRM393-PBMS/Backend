...
## Local PRM393 integration

Read root `AGENTS.md` and `.agents/skills/prm393-backend/SKILL.md` for project-specific constraints before applying these general examples. Load only relevant rule files.

This repository uses Prisma 6.4.1, not TypeORM. Treat TypeORM examples as conceptual illustrations; use the injected PrismaService and the local `prm393-backend/references/prisma-6.md` guidance for persistence. Do not install TypeORM or add repository layers, events, caching, queues, microservices or dependencies merely to satisfy a general rule. Check installed NestJS 12 APIs before copying examples. Database transactions cannot roll back external HTTP/payment side effects; keep external calls out of database transactions and design idempotency or an outbox when the task needs it.

Source: https://github.com/Kadajett/agent-nestjs-skills/tree/main/skills/nestjs-best-practices . Local adaptation adds this section; upstream rules are retained as references.
